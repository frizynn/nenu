...
describe("ChatFilesBrowser", () => {
  ...
  it("pages to the beginning before claiming the conversation is scanned and deduplicates references", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: "Files and photos" }));
    ...
    await userEvent.click(screen.getByRole("button", { name: /Photos · 1/ }));
  ...
  it("opens a selected reference through FilePreviewProvider's context", async () => {
    ...
    expect(screen.queryByRole("dialog", { name: "Files and photos" })).not.toBeInTheDocument();
  });
  ...
  it("shows an empty state and retries failed older-page reads", async () => {
    ...
    expect(screen.getByText("No files in this conversation")).toBeInTheDocument();
  });
  ...
  it("dismisses stale paths immediately when the pane scope changes", async () => {
    ...
    await waitFor(() => expect(screen.queryByRole("dialog", { name: "Files and photos" })).not.toBeInTheDocument());
