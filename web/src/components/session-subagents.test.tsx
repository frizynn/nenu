...
it("opens a child's transcript without sending input or navigating away", async () => {
  ...
  await user.click(await screen.findByRole("button", { name: "Subagents (1)" }));
...
it("does not present stale running state after refresh fails", async () => {
  ...
  await screen.findByRole("button", { name: "Subagents (1)" });
  fail = true;
  await user.click(screen.getByRole("button", { name: "Subagents (1)" }));
